...
//write IPs into array 
	...
	while ($i2 < $ip_numbers_in_file) {
		...
		$pos = strpos($ip_array_split[1], "17170"); //position of 17170
		$bool_pos=is_bool($pos); //custom ports (!= 17170) not allowed.
		
		if (!$bool_pos) {
			$ip_array_split = str_replace("17170", "", $ip_array_split[1]); //remove port
